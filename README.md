...
- 이용 설명 페이지
  ...
  - 단계별 서비스 이용 안내 제공
- 유출 안내문 분석 기능
  - 사용자가 개인정보 유출 문자, 이메일, 공지문을 입력하면 AI가 기업명, 서비스명, 사고 키워드, 유출 항목을 추출한다.
  - 입력 내용은 분석 전 개인정보 마스킹 처리를 거치며, 검색 API를 통해 관련 기사, 공식 공지, 웹문서를 함께 확인한다.
  - AI는 입력문과 검색 결과를 종합하여 사고 관련성, 위험도, 2차 피해 가능성을 분석한다.
  - 분석 결과로 유출 항목, 위험 유형, 위험도, 우선순위별 대응 체크리스트를 제공한다.

- 의심 문자 분석 기능
  - 유출 사고 이후 받은 보상금, 환불, 본인인증 요구 문자 등 의심 문자를 입력할 수 있다.
  - AI는 링크 포함 여부, 요구 행동, 문장 패턴 등을 기준으로 피싱·스미싱·사칭 가능성을 분석한다.
  - 분석 결과로 위험 유형과 위험도, 사용자가 주의해야 할 행동을 안내한다.

- 결과 활용 기능
  - 분석 내용을 바탕으로 가족에게 전달할 수 있는 쉬운 공유용 안내문을 생성한다.
